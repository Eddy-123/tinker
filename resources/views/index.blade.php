                        ...
                        @foreach ($films as $film)
                            <tr @if ($film->deleted_at) class="has-background-grey-lighter" @endif>
                                ...
                                <td>
                                    @if ($film->deleted_at)
                                        <form action="{{ route('films.restore', $film->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button class="button is-primary">Restaurer</button>
                                        </form>
                                    @else
                                        <a class="button is-primary" href="{{ route('films.show', $film->id) }}">
                                            Voir
                                        </a>
                                    @endif
                                </td>
                                <td>
                                    @if ($film->deleted_at)
                                    @else
                                        <a class="button is-warning" href="{{ route('films.edit', $film->id) }}">
                                            Modifier
                                        </a>
                                    @endif
                                </td>
